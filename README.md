...
```
frontend/   Vite + React 19 + TypeScript (port 5173, proxies /api to the backend)
backend/    FastAPI + pydantic, managed with uv (port 8000)
backend/data/   authored content: agents, learning items, personas, mappings (JSON)
backend/var/    runtime state: learning.db (SQLite), interactions.jsonl, feedback.jsonl
```
