...
NEXT_STEPS = [
    {"label": "Submit an intake request for a new agent", "href": "/intake/new"},
    {"label": "Learn how to build an agent", "href": "/learning?path=builders"},
    {"label": "Ask the community", "href": "/community/forums"},
]
...
def _build_carousels(persona: Persona | None) -> list[Carousel]:
    ...
    for c in store.carousels:
        if c.rule.type == "persona":
            ...
            items = sorted((a for a in agents if a.category == c.rule.value), key=lambda a: -a.popularity)
        elif c.rule.type == "featured":
            items = sorted((a for a in agents if a.featured or a.popularity >= 80), key=lambda a: -a.popularity)
        else:  # recent
            items = sorted(agents, key=lambda a: a.updated_at, reverse=True)[:8]
        if items:
            out.append(Carousel(id=c.id, title=c.title, subtitle=c.subtitle, agents=items))
    return out


def _derived_persona_id() -> str:
    from ..identity import derive_persona, load_profile  # local import avoids a cycle at import time

    return derive_persona(load_profile()).id


@router.get("/home", response_model=MarketplaceHome)
def marketplace_home(persona: str | None = Query(default=None)) -> MarketplaceHome:
    p = store.persona(persona or _derived_persona_id())
...
def curation(persona: str | None = Query(default=None)) -> dict:
    ...
    for a in store.agents:
        ...
        rows.append({"agent_id": a.id, "name": a.name, "total": round(sum(parts.values()), 2),
    ...
    return {
        ...
        "excluded": ["behavioural footprints (collected, not used for ranking in the MVP)"],
        "ranked": rows,
    }
...
def nlp_search(req: SearchRequest) -> SearchResponse:
    agents = store.agents
    persona = store.persona(req.persona or _derived_persona_id())
    intent, engine = search.understand(req.query, agents)
    results = search.rank(req.query, intent, agents, persona=persona, domain=req.domain, limit=req.limit)
    context_store.record_event(
        {
            ...
            "topics": sorted({d for m in results for d in m.agent.business_domains} | set(intent.domains)),
...
def agent_docs(agent_id: str) -> DocPage:
    ...
    return DocPage(agent_id=agent.id, agent_name=agent.name, title=f"{agent.name} documentation", markdown=md, source_url=agent.documentation_url)
...
def agent_architecture(agent_id: str) -> DocPage:
    ...
    title = f"{agent.architecture_pattern} pattern" if agent.architecture_pattern else "Architecture"
    return DocPage(agent_id=agent.id, agent_name=agent.name, title=title, markdown=md, source_url=agent.architecture_url)


@router.get("/agents/{agent_id}/learning", response_model=list[ItemWithProgress], deprecated=True)
